...
filterView.onclick = function () {
    var searchBy = "";
    $("input[name='searchBy']:checked").each(function(j) {
        if (j >= 0) {
            searchBy += $(this).val() + ","
        }
    });
    if (filterView.checked){
        // alert("ok");
        ajaxInSort(1,searchText.value,searchBy,totalPage.innerText,"view");
    } else {
        ajaxInSort(1,searchText.value,searchBy,totalPage.innerText,"");
    }
};
function ajaxInSort(page,searchText,searchBy,totalPage,sortBy) {
    page = 1;
    $.ajax({
        type: "post",
